...
string findWordbyPrefInner(TrieNode* node, string& res, string& pref, string& resInner) // рекурсивная функция для вывода слов в поддереве первого символа префикса
{
    for (int i = 0; i < ALPHABET_SIZE; i++)
    {
        if (node->children[i])
        {
            if (node->isEndOfWord) // если конец слова = добавляем пробел(разделение слов) и найденную часть как начальную часть большего слова
            {
                res += ' ';
                res += resInner;
            }
...
void findWordbyPref(TrieNode* root, string& res, string& pref) // головная функция поиска слов по префиксу
{
    res += pref[0];
    string resInner = res;
    bool prefExists = search(root, pref); // для начала проверяем есть ли в начале вообще введенный префикс

    if (!prefExists) // проверка - есть ли префикс в дереве. Если нет - выводим что его нет, очищаем переменную результат, выходим из функции
    {
    ...
    struct TrieNode* node = searchNodePref(root, pref); //находим вершину, соответствующую первому!! символу префикса
    
    findWordbyPrefInner(node, res, pref, resInner); // вызов рекурсивной функции с вершиной, соответствующей первому!! символу префикса
}
